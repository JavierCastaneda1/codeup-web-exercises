...
function getTallestHamster(arr){
    ...
    arr.forEach(function(el){           //for each piece of the array check...
        if (el.heightInMM > tallest.heightInMM){    //if elements height is greaters than tallest(variable).
            tallest = el;
        }
    });
    return tallest;                    //return tallest element.
}
...
function getAverageHeight(array){
    let totalHeight = 0;        //declare a total height set to zero as a baseline.
    for (let element of array){         //let elements of the input array..
...
console.log(getAverageHeight(hamstersTwo)); // should return 93.2;
